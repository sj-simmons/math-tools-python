...
'''Provides a polynomial class.

 A polynomial class implemented using composition (with list objects). The underlying
 concrete representation of a polynomial is a list of its coefficients of length equal 
 to its degree.                                                           
'''
...
class Polynomial:
    ...
    '''Implements addition, subtraction, multiplication, and evaluation of polynomials.  
      ...
      Notes: 
         ...
         p = Polynomial([]) is the 0 polynomial (not None) with p.degree = None.

         Be careful: comparison is just comparison of lists which may or 
         may not be what the user wants on the level of polynomials;
         similarly, regarding max, min, sort, reverse, etc

      TODO:

         Implement comparisons sensibly, if possible. 
         Implement copy and deepcopy (so we can have polynomials over polynomials).
    ''' 

    ''' implementation notes: 
        
        Some methods use local variables (which is more efficient than 
        accessing instance variables) whenever, in general, doing so 
        ...
        If poly is instantiated using poly = Polynomial([a_0,a_1,...,a_n]), then
         ...
         2. len(self) = self._degree  
      
         3. self._coeffs is a list of the coefficients of self  
    '''             
    ...
    def _add(self, other):  
        ...
        if other._degree is None:
            return self 
        minDegree = min([self._degree, other._degree])
        if minDegree == self._degree:
            shorter = self; longer = other
        else:
            shorter = other; longer = self
        return Polynomial([ shorter[i] + longer[i] for i in range( minDegree + 1)] \
                                               + longer[ minDegree + 1 : ])
    def __neg__(self):
    ...
    def __len__(self):
        """Return number of coefficients of a Polynomial, which is its degree.""" 

        return self._degree + 1

    def __eq__(self,other):
        """Return true if the two Polynomials have the same length and equal coefficients.
        ...
        >>> print("Is",p1,"==", p2,"?", p1 == 2 * p2) 
        Is 1 + 2x + 3x^2 == 1/2 + x + 3/2x^2 ? True
